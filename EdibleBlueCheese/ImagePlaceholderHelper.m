...
@implementation ImagePlaceholderHelper

static ImagePlaceholderHelper *sharedObject = nil;

+ (ImagePlaceholderHelper *)sharedInstance
{
...
- (UIImage *)placerholderImageWithSize:(CGSize)size text:(NSString *)text fillColor:(UIColor *)fillColor
{
    ...
    if (!cachedImage)
    {
        ...
        UIFont *font = [UIFont fontWithName:@"GillSans" size:MIN(size.width, size.height) / 8];
        CGRect rect = CGRectMake(0, 0, size.width, size.height);
